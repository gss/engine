...
class Query extends Command
  ...
  getCanonicalPath: (continuation, compact) ->
    Query.PopDirectives ||= new RegExp("" +
        "@[^@"   + this.DESCEND   + "]+" +
        this.DESCEND + "$", "g")
    while (replaced = continuation.replace(Query.PopDirectives, '')) != continuation
      continuation = replaced
      
    bits = @delimit(continuation).split(@DESCEND)
    last = bits[bits.length - 1]
    Query.RemoveForkMarks ||= new RegExp("" +
        "([^"   + @PAIR   + ",@])" +
        "\\$[^\[" + @ASCEND + "]+" +
        "(?:"   + @ASCEND + "|$)", "g")
    last = bits[bits.length - 1] = last.replace(Query.RemoveForkMarks, '$1')
