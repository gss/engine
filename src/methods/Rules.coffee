  ...
  "if":
    ...
    update: (operation, continuation, scope, meta, ascender, ascending) ->
      ...
      if !watchers.length || @indexOfTriplet(watchers, operation, continuation, scope) == -1
        ...
        @solved.watch null, path, operation, continuation, scope


      condition = ascending && (typeof ascending != 'object' || ascending.length != 0)
      old = @solved[path]
      if @solved.set(null, path, condition ? null) == condition
        index = condition && 2 || 3
        ...
        unless old == undefined
          @solved.clean(path, continuation, operation.parent, scope)
    ...
    capture: (result, operation, continuation, scope, meta) ->
      console.log('capture', result, operation.index, continuation, @displayName, 666)
