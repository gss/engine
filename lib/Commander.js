...
var Commander, bindRoot,
  __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
  __slice = [].slice;

bindRoot = function(root, query) {
  var isContextBound, isMulti;
...
Commander = (function() {
  ...
  Commander.prototype.registerSpawn = function(node) {
    if (!node.isQueryBound) {
      return this.engine.registerCommand(node);
    } else {
  ...
  Commander.prototype.spawn = function(node) {
    var context_id, q, queries, ready, _i, _j, _len, _len1, _ref, _results;
    ...
    if (ready) {
      if (node.isContextBound) {
        _ref = node.parentRule.getContextQuery().lastAddedIds;
        _results = [];
        for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
          context_id = _ref[_j];
          _results.push(this._spawnCommandFromBase(node, context_id));
        }
        return _results;
      } else {
        return this._spawnCommandFromBase(node);
      }
    }
    /* TODO
  ...
  Commander.prototype._spawnCommandFromBase = function(command, contextId) {
    ...
    if (hasPlural) {
      ...
      return this.engine.registerCommands(commands);
    } else {
  ...
  Commander.prototype['get$'] = function(root, prop, query) {
    var isContextBound, isMulti, selector,
      _this = this;
    if (query === 'window') {
  ...
  Commander.prototype['$reserved'] = function(root, sel) {
    var engine, parentRule, query;
    query = null;
    if (sel === 'window') {
      return 'window';
    } else if (sel === '::this' || sel === 'this') {
      engine = this.engine;
