...
Mutation = (function() {
  ...
  Mutation.prototype.Observer = (typeof window !== "undefined" && window !== null) && (window.MutationObserver || window.WebKitMutationObserver || window.JsMutationObserver);

  Mutation.prototype.connect = function(temporary) {
    var _ref;
  ...
  Mutation.prototype.disconnect = function(temporary) {
    var _ref;
    if (temporary && window.JsMutationObserver === this.Observer) {
      return;
    }
  ...
  Mutation.prototype.filter = function(mutation) {
    var parent;
  ...
  Mutation.prototype.filterNode = function(target) {
    if (target._gss) {
      return false;
    }
    return true;
  ...
  Mutation.prototype.solve = function(mutations) {
    var result;
  ...
  Mutation.prototype.onChildList = function(target, mutation) {
    var added, allAdded, allChanged, allMoved, allRemoved, attribute, changed, changedTags, child, el, firstNext, firstPrev, id, index, j, kls, moved, next, node, parent, prev, prop, queries, removed, tag, update, value, values, _base, _i, _j, _k, _l, _len, _len1, _len10, _len11, _len12, _len13, _len2, _len3, _len4, _len5, _len6, _len7, _len8, _len9, _m, _n, _o, _p, _q, _r, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _s, _t, _u, _v;
    added = [];
    removed = [];
    ...
    queries = this.engine.queries;
    while ((prev = prev.previousSibling)) {
      if (prev.nodeType === 1) {
        if (firstPrev) {
          queries.match(prev, '+', void 0, '*');
          queries.match(prev, '++', void 0, '*');
          firstPrev = false;
        }
        queries.match(prev, '~', void 0, changedTags);
        queries.match(prev, '~~', void 0, changedTags);
        next = prev;
      }
    }
    while ((next = next.nextSibling)) {
      if (next.nodeType === 1) {
        if (firstNext) {
          queries.match(next, '!+', void 0, '*');
          queries.match(next, '++', void 0, '*');
          firstNext = false;
        }
        queries.match(next, '!~', void 0, changedTags);
        queries.match(next, '~~', void 0, changedTags);
      }
    }
    queries.match(target, '>', void 0, changedTags);
    allAdded = [];
    allRemoved = [];
    allMoved = [];
    moved = [];
    for (_l = 0, _len3 = added.length; _l < _len3; _l++) {
      child = added[_l];
      queries.match(child, '!>', void 0, target);
      allAdded.push(child);
    ...
    parent = target;
    while (parent) {
      queries.match(parent, ' ', void 0, allChanged);
      for (_s = 0, _len10 = allChanged.length; _s < _len10; _s++) {
        child = allChanged[_s];
        queries.match(child, '!', void 0, parent);
      }
      for (prop in update) {
        values = update[prop];
        for (_t = 0, _len11 = values.length; _t < _len11; _t++) {
          ...
          if (prop.charAt(1) === '$') {
            ...
            queries.match(parent, prop, void 0, value);
          }
        }
      }
  ...
  Mutation.prototype.index = function(update, type, value) {
    var group;
  ...
  Mutation.prototype.onCharacterData = function(target, parent) {
    var id, _ref;
    if (parent == null) {
  ...
  Mutation.prototype.onAttributes = function(target, name, changed) {
  ...
  Mutation.prototype.match = function(node, group, qualifier, changed, continuation) {
  ...
  Mutation.prototype.qualify = function(operation, continuation, scope, groupped, qualifier, fallback) {
