...
Command = (function() {
  ...
  Command.prototype.solve = function(engine, operation, continuation, scope, ascender, ascending) {
    ...
    switch (typeof (result = this.retrieve(domain, operation, continuation, scope, ascender, ascending))) {
      ...
      case 'boolean':
        if (result) {
          result = void 0;
          continuation = this.getScopePath(scope, continuation);
        } else {
          return;
        }
    }
  ...
  Command.prototype.connect = function(engine, operation, continuation, scope, args, ascender) {
    if ((ascender != null) && continuation[continuation.length - 1] !== this.DESCEND) {
      return this.continuate(continuation, this.PAIR);
    }
  };

  Command.prototype.fork = function(engine, continuation, item) {
    return this.continuate(continuation + engine.identity(item), this.ASCEND);
  };
  ...
  Command.getRoot = function(operation) {
  ...
  Command.prototype.getScopePath = function(scope, continuation) {
  ...
  Command.prototype.getParentScope = function(scope, continuation) {
  ...
  Command.prototype.getCanonicalPath = function(continuation, compact) {
    var bits, last;
  ...
  Command.prototype.CanonicalizeRegExp = new RegExp("" + "([^" + Continuation.prototype.PAIR + ",])" + "\\$[^" + Continuation.prototype.ASCEND + "]+" + "(?:" + Continuation.prototype.ASCEND + "|$)", "g");

  Command.prototype.TrimContinuationRegExp = new RegExp("[" + Continuation.prototype.ASCEND + Continuation.prototype.DESCEND + Continuation.prototype.PAIR + "]$");

  Command.prototype.getVariants = function(path) {
