...
Mutations = (function() {
  ...
  Mutations.prototype.clean = function(id, continuation) {
    var child, contd, index, path, result, watcher, watchers, _i, _j, _len, _len1;
    ...
    if (old && old.length) {
      ...
      for (_i = 0, _len = old.length; _i < _len; _i++) {
        ...
        if (!result || old.indexOf.call(result, child) === -1) {
          this.engine.clean(this.references.identify(child), path);
    ...
    for (index = _i = 0, _len = watchers.length; _i < _len; index = _i += 2) {
      ...
      if (groupped = operation[group]) {
        if (qualifier) {
          ...
          if (changed.nodeType) {
            if (indexed = groupped[changed.tagName] || groupped["*"]) {
              if (queries.indexOf(operation) === -1) {
                queries.push(operation, watchers[index + 1]);
              }
            }
