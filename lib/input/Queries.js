...
Queries = (function() {
  ...
  Queries.prototype.write = function(queries) {
    ...
    for (index = _i = 0, _len = queries.length; _i < _len; index = _i += 3) {
      ...
      this.output.read(query, void 0, continuation, void 0, void 0, scope);
    }
    return this;
  };
    ...
    if (result = this[path]) {
      ...
      if (result.length !== void 0) {
        for (_i = 0, _len = result.length; _i < _len; _i++) {
          child = result[_i];
          this.engine.context.remove(child, path);
        }
      } else {
  ...
  Queries.prototype.filter = function(node, result, operation, continuation, subscope) {
    ...
    subscope || (subscope = node || this.engine.scope);
    if (id = this.references.identify(subscope)) {
  ...
  Queries.prototype.match = function(queries, node, group, qualifier, changed) {
    var change, continuation, groupped, id, index, operation, subscope, watchers, _i, _j, _len, _len1;
    if (!(id = node._gss_id)) {
      return;
    }
    if (!(watchers = this._watchers[id])) {
    ...
    for (index = _i = 0, _len = watchers.length; _i < _len; index = _i += 3) {
      ...
      if (groupped = operation[group]) {
        ...
        if (qualifier) {
          this.qualify(queries, operation, continuation, subscope, groupped, qualifier);
        } else if (changed.nodeType) {
          this.qualify(queries, operation, continuation, subscope, groupped, changed.tagName, '*');
        } else {
          for (_j = 0, _len1 = changed.length; _j < _len1; _j++) {
            change = changed[_j];
            this.qualify(queries, operation, continuation, subscope, groupped, change.tagName, '*');
          }
        }
      }
    }
