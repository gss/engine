...
Queries = (function() {
  ...
  Queries.prototype.unpair = function(continuation, operation, scope, result) {};

  Queries.prototype.pair = function(continuation, operation, scope, result) {
    var collection, element, match, node, plurals, schedule, _base, _i, _len, _name;
    ...
    if (operation.path !== match[3]) {
      ...
      if (this.engine.isCollection(result)) {
        for (_i = 0, _len = result.length; _i < _len; _i++) {
          node = result[_i];
          this.add(node, match[1] + '–' + operation.path, operation, scope);
        }
      } else {
        this.add(result, match[1] + '–' + operation.path, operation, scope);
      }
    }
    debugger;
  ...
  Queries.prototype.update = function(node, args, result, operation, continuation, scope) {
    ...
    if (node !== scope) {
      ...
      if (group = (_ref = this.updated) != null ? _ref[query] : void 0) {
        if (query.indexOf('$container0 .b') > -1) {
          debugger;
        }
        result = group[0];
        if (this[path] == null) {
