...
Queries = (function(_super) {
  ...
  Queries.prototype.unwatch = function(id, continuation, plural, quick) {
    ...
    if (continuation !== true) {
      ...
      if (typeof id !== 'object') {
        this.unpair(continuation, this.engine.ids[id]);
      }
    }
    index = 0;
  ...
  Queries.prototype.remove = function(id, continuation, operation, scope, manual, plural) {
    var collection, node;
    this.engine.console.row('remove', id.nodeType && this.engine.identity.get(id) || id, continuation);
    if (typeof id === 'object') {
      node = id;
      id = this.engine.identity.get(id);
    } else {
      node = this.engine.ids[id];
    }
    if (continuation) {
      collection = this.get(continuation);
  ...
  Queries.prototype.repair = function(path, key, operation, scope, collected) {
    ...
    if (!rightNew && collected) {
      rightNew = this.get(path + this.engine.identity.get(leftNew[0] || leftOld[0]) + '→' + key);
    }
    rightNew || (rightNew = []);
  ...
  Queries.prototype.pair = function(continuation, operation, scope, result) {
    ...
    element = match[2] ? this.engine.ids[match[2]] : this.get(match[1]);
    if (this._repairing !== void 0) {
  ...
  Queries.prototype.update = function(node, args, result, operation, continuation, scope) {
    ...
    if (id = this.engine.identity.get(node)) {
      watchers = (_base = this._watchers)[id] || (_base[id] = []);
      if (this.engine.indexOfOfTriplet(watchers, operation, continuation, scope) === -1) {
