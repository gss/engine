...
Engine = (function() {
  function Engine(o) {
    ...
    this.handleMutations = __bind(this.handleMutations, this);
    this.updateChildList = __bind(this.updateChildList, this);
    this.loadASTs = __bind(this.loadASTs, this);
  ...
  Engine.prototype.updateIfNeeded = function() {
    ...
    if (this.needsUpdate) {
      if (!this.ASTs) {
        throw new Error("ASTs needed for update");
      }
      this.run(this.ASTs);
      this.ASTs = null;
  ...
  Engine.prototype.beforeLayout = function() {};
  ...
  Engine.prototype.layoutIfNeeded = function() {
    ...
    if (this.needsLayout) {
      return this.layout();
    } else if (!this.waitingToLayoutSubtree) {
  ...
  Engine.prototype.loadASTs = function() {
  ...
  */
  ...
  Engine.prototype.handleMutations = function(mutations) {
    var childListTrigger, gid, invalidMeasures, m, target, trigger, _i, _len;
    if (mutations == null) {
      mutations = [];
    }
    LOG(this.id, ".handleMutations()");
    trigger = false;
    invalidMeasures = [];
    ...
    */
    ...
    if (!GSS.needsDisplay && !GSS.needsLayout && !GSS.needsDisplay) {
      if (trigger) {
        this.commander.handleInvalidMeasures(invalidMeasures);
      }
      return childListTrigger = this.updateChildList();
    }
  };
