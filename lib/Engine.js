...
Engine = (function(_super) {
  ...
  Engine.prototype.elVar = function(el, key, selector, tracker2) {
    ...
    if (key === 'left') {
      ...
      this.registerCommand(['varexp', this.plus(this.elVar(el, 'y', selector), this.elVar(el, 'height', selector))]);
    } else if (key === 'right') {
      this.registerCommand(['varexp', this.plus(this.elVar(el, 'x', selector), this.elVar(el, 'width', selector))]);
    } else if (key === 'center-y') {
      this.registerCommand(['varexp', this.plus(this.elVar(el, 'y', selector), this.divide(this.elVar(el, 'height', selector), 2))]);
    } else if (key === 'center-x') {
      this.registerCommand(['varexp', this.plus(this.elVar(el, 'x', selector), this.divide(this.elVar(el, 'width', selector), 2))]);
    } else {
      varid = "$" + gid + ("[" + key + "]");
