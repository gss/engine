...
Engine = (function() {
  ...
  Engine.prototype["command-var"] = function(id, prop, elements) {
    var el, newcommands, _i, _len;
    newcommands = [];
    if (elements instanceof Array) {
      for (_i = 0, _len = elements.length; _i < _len; _i++) {
        el = elements[_i];
        this.commandsForWorker.push("var", el.id + prop);
      }
      elements.onadd(function(newElements) {
        var _j, _len1, _results;
        _results = [];
        for (_j = 0, _len1 = elements.length; _j < _len1; _j++) {
          el = elements[_j];
          _results.push(this.commandsForWorker.push("var", el.id + prop));
        }
        return _results;
      });
    } else {
      this.commandsForWorker.push(["var"].concat(__slice.call(arguments)));
    }
    return newcommands;
  };
  ...
  Engine.prototype["command-$class"] = function(className) {
    var _this = this;
    return this._registerLiveNodeList("." + className, function() {
      return _this.container.getElementsByClassName(className);
    });
  };
