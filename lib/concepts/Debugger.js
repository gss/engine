...
Debugger = (function() {
  ...
  Debugger.prototype.domains = function(domains) {
    ...
    return this.list.innerHTML = domains.map(function(d) {
      ...
      return "<domain for=\"" + d.uid + "\" " + (this.console.level <= 1 && 'hidden') + " class=\"" + (d.displayName.toLowerCase()) + "\">" + d.constraints.length + "</domain>";
    }).join('');
  };

  Debugger.prototype.ruler = function(element, path, value, x, y, width, height, inside) {
    var a, b, bits, constraint, distances, domain, id, konst, other, property, ruler, sheet, _i, _j, _k, _len, _len1, _len2, _ref, _ref1, _ref2, _ref3, _ref4;
    if (!this.rulers) {
